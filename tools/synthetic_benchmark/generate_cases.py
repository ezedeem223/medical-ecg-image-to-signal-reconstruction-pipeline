...
# Fixed per-distortion seed offsets.  Must NOT use hash() — Python salts hash()
# per process, making it non-deterministic across runs.  These integers are
# stable constants: identical inputs always produce identical output files.
_DISTORTION_OFFSET: dict[str, int] = {
    "clean":        0,
    "blur":         1,
    "noise":        2,
    "low_contrast": 3,
...
def generate_cases(
    ...
    for case_idx in range(num_cases):
        ...
        for dist_name in distortions:
            dist_seed = seed + _DISTORTION_OFFSET[dist_name]
