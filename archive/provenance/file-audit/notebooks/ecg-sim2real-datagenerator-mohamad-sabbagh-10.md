# File Audit: `ecg-sim2real-datagenerator-mohamad-sabbagh (10).ipynb`
...
| --- | --- |
| Review order | 11 |
| Original path | ecg-sim2real-datagenerator-mohamad-sabbagh (10).ipynb |
| Report path | docs/file-audit/notebooks/ecg-sim2real-datagenerator-mohamad-sabbagh-10.md |
| SHA-256 | 9d74fe69682c2684f36ea2375df8bf7538b6111cb903a09ee9f7c9f815b1374d |
| Size (bytes) | 95,074 |
| Modified (UTC) | 2026-04-18T19:34:25.115343+00:00 |
...
- Previous notebook: `ecg-sim2real-datagenerator-mohamad-sabbagh (9).ipynb`
- Compared with `ecg-sim2real-datagenerator-mohamad-sabbagh (9).ipynb`, cell count changed from 23 to 5, stored output types changed from 49 to 8, and exact shared cell sources = 4.
- New imports: None
...
- Removed model refs: `YOLO: yolov8x.pt`, `os.path.exists: best_model_phase9.pth`, `os.path.exists: best_model_real_data.pth`, `os.path.exists: runs/detect/ecg_combat_detector/weights/best.pt`, `torch.load: best_model_phase9.pth`, `torch.load: best_model_real_data.pth`, `torch.save: best_model_phase10.pth`, `torch.save: best_model_phase2.pth`
- New first-line titles: `# --- الخلية 22: المحرك البلاتيني (Phase 14+15) مع المسارات الدائمة - [FINAL & FIXED] ---`
- Removed first-line titles: `# --- الخلية 15: المحرك الهندسي المتقدم (Advanced Geometry Engine) ---`, `# --- الخلية 17: اختبار يدوي (مصحح) ---`, `# --- الخلية 18: إعداد بيانات YOLO القتالية (Combat Mode) - [M4: UPDATED] ---`, `# --- الخلية 19: تدريب YOLOv8x القتالي [M4: COMBAT TRAINING] ---`, `# --- الخلية 20: مصنع البيانات المصفح والتدريب التكميلي (Phase 2: Hardening) ---`, `# --- الخلية 22: المحرك البلاتيني (Phase 14: Signal Processing + Phase 15: TTA) - [FINAL BUILD] ---`, `# --- الخلية 4: الضبط الدقيق على الواقع (Phase 10: Real World Fine-Tuning) - [M10: PSEUDO] ---`, `# --- الخلية 6---`, `# --- الخلية 7 ---`, `# --- الخلية 8:  ---`, `# --- الخلية 9:  ---`, `# --الخلية 5 ---`, `# الخلية 10`, `# الخلية 11`, `# خلية 12`, `# خلية 13`, `# خلية 14`, `# خلية 16`, `# خلية 21`
...
| Cell type | code |
| Execution count | 4 |
| First non-empty line | `# --- الخلية 22: المحرك البلاتيني (Phase 14+15) مع المسارات الدائمة - [FINAL & FIXED] ---` |
...
- stream[stderr] with 1 line(s); excerpt: 100%|██████████| 24/24 [05:06<00:00, 12.76s/it]
- stream[stdout] with 2 line(s); excerpt: 💾 الحفظ النهائي (Platinum Edition)... 🏆 تمت المهمة! النوت بوك الآن يقرأ من Input وجاهز للتسليم.
