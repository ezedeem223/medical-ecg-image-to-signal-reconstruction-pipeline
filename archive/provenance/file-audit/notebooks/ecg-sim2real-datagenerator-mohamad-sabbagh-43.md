# File Audit: `ecg-sim2real-datagenerator-mohamad-sabbagh (43).ipynb`
...
| --- | --- |
| Review order | 44 |
| Original path | ecg-sim2real-datagenerator-mohamad-sabbagh (43).ipynb |
| Report path | docs/file-audit/notebooks/ecg-sim2real-datagenerator-mohamad-sabbagh-43.md |
| SHA-256 | fabe11c60594d31eaf3e3cc4cd2e03bda33511724cbbb241fc8a66749a7efcc4 |
| Size (bytes) | 46,676 |
| Modified (UTC) | 2026-04-18T20:16:56.977917+00:00 |
...
- Previous notebook: `ecg-sim2real-datagenerator-mohamad-sabbagh (42).ipynb`
- Compared with `ecg-sim2real-datagenerator-mohamad-sabbagh (42).ipynb`, cell count changed from 26 to 5, stored output types changed from 7 to 11, and exact shared cell sources = 2.
- New imports: `importlib`, `pyarrow`, `site`, `subprocess`
- Removed imports: `albumentations`, `io`, `matplotlib.pyplot`, `random`, `scipy.fft`, `shutil`, `skimage.measure`, `skimage.transform`, `sklearn.model_selection`, `torch.nn`, `torch.optim.lr_scheduler`, `torch.utils.data`, `warnings`, `wfdb`
- New model refs: None
- Removed model refs: `YOLO: yolov8x.pt`, `os.path.exists: /kaggle/input/ecg-final-models/best_model_phase10.pth`, `os.path.exists: best_model_phase9.pth`, `os.path.exists: best_model_real_data.pth`, `os.path.exists: runs/detect/ecg_combat_detector/weights/best.pt`, `torch.load: /kaggle/input/ecg-final-models/best_model_phase10.pth`, `torch.load: best_model_phase9.pth`, `torch.load: best_model_real_data.pth`, `torch.load: temp_deeplab_base.pth`, `torch.load: temp_effnet_base.pth`, `torch.save: best_model_deeplab_ph10.pth`, `torch.save: best_model_efficientnet_ph10.pth`, `torch.save: best_model_phase10.pth`, `torch.save: best_model_phase2.pth`, `torch.save: temp_deeplab_base.pth`, `torch.save: temp_effnet_base.pth`
- New first-line titles: `# --- Cell 2: Offline install/import fix (NO internet) ---`, `# --- الخلية 1: التثبيت الذكي (Smart Offline Install) ---`, `# =========================`
- Removed first-line titles: `# --- الخلية 15: المحرك الهندسي المتقدم (Advanced Geometry Engine) ---`, `# --- الخلية 17: اختبار يدوي (مصحح) ---`, `# --- الخلية 18: إعداد بيانات YOLO القتالية (Combat Mode) - [M4: UPDATED] ---`, `# --- الخلية 19: تدريب YOLOv8x القتالي [M4: COMBAT TRAINING] ---`, `# --- الخلية 1: التثبيت وإعادة التشغيل ---`, `# --- الخلية 20: مصنع البيانات المصفح والتدريب التكميلي (Phase 2: Hardening) ---`, `# --- الخلية 2: الاستيرادات المحدثة (Updated Imports) ---`, `# --- الخلية 4: الضبط الدقيق على الواقع (Phase 10: Real World Fine-Tuning) - [M10: PSEUDO] ---`, `# --- الخلية 6---`, `# --- الخلية 7 ---`, `# --- الخلية 8:  ---`, `# --- الخلية 9:  ---`, `# --- الخلية الإضافية 1 (النسخة الآمنة): تدريب المنافس الأول (EfficientNet-B3) ---`, `# --- الخلية الإضافية 4.8 تدريب النموذج الثاني: تدريب المنافس الثاني (DeepLabV3+) ---`, `# --الخلية 5 ---`, `# الخلية 10`, `# الخلية 11`, `# بديل الخلية الرباعه من اجل التعريفات 4.5`, `# خلية 12`, `# خلية 13`, `# خلية 14`, `# خلية 16`, `# خلية 21`, `# خلية 23`
...
- stream[stderr] with 1 line(s); excerpt: Scanning Lengths: 100%|██████████| 75000/75000 [00:00<00:00, 916956.12it/s]
- stream[stdout] with 2 line(s); excerpt: ✅ Lengths mapped for 2 patients. 🗂️ Indexing images...
- stream[stderr] with 1 line(s)
- stream[stdout] with 9 line(s); excerpt: ✅ Indexed images: 8,795 ✅ fs_map loaded: 2 items ⚙️ Loading models (Ensemble Mode)... ✅ YOLO loaded: /kaggle/input/ecg-final-models1/best.pt ✅ Ensemble Model 1: ResNet50 Loaded. ✅ ...
- stream[stderr] with 1 line(s); excerpt: Processing Rows: 100%|██████████| 75000/75000 [00:04<00:00, 17875.47it/s]
